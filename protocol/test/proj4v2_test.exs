defmodule Proj4v2Test do
  ...
  doctest Proj4v2
   ...
   test "Proof of Work" do
     ...
     {_, ledger, _, _, _, _, _} = Miner.get_miner_information(List.first(miner_list))
  ...
  test "two invalid transaction: 0->1 75 btc, 0->2 65 btc, then 0->2 15 btc, 0->1 25 btc" do
    IO.puts("Test 9")
    {user_list, miner_list} = Proj4v2.init(10, 5)
    User.transaction(Enum.at(user_list, 0), miner_list, Enum.at(user_list, 1), 75)
    User.transaction(Enum.at(user_list, 0), miner_list, Enum.at(user_list, 2), 65)

    Miner.miner_mining(miner_list)
    :timer.sleep(1000)

    User.transaction(Enum.at(user_list, 0), miner_list, Enum.at(user_list, 2), 15)
    User.transaction(Enum.at(user_list, 0), miner_list, Enum.at(user_list, 1), 25)
    ...
    balance = Miner.getBalance(List.first(miner_list))
    {_, _, public_key0} = User.get_user_information(Enum.at(user_list, 0))
    assert balance[Base.encode64(public_key0)] == 10
    {_, _, public_key1} = User.get_user_information(Enum.at(user_list, 1))
    assert balance[Base.encode64(public_key1)] == 175
    {_, _, public_key2} = User.get_user_information(Enum.at(user_list, 2))
    assert balance[Base.encode64(public_key2)] == 115
  end
